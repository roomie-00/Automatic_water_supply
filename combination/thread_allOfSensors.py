...
def buttons_thread(t1, t2):
    try:
        ...
        GPIO.add_event_detect(volume_button, GPIO.FALLING, callback=period_button_callback)
    finally:
        GPIO.cleanup()
...
try:
    t = threading.Thread(target=buttons_thread, args=(1,2))
    t.start()
    t.join()
    while True:
        val = readChannel(0)
